...
class TransformerAttention(nn.Module):
    def __init__(self, dim, num_heads=2, num_layers=1):
    ...
    def forward(self, x, batch_idx=0):
        for attn, norm, ffn, norm2 in zip(self.layers, self.norms, self.ffns, self.norm2s):
            ...
            if torch.isnan(attn_output).any() or torch.isinf(attn_output).any():
                logger.warning(f"Batch {batch_idx}: TransformerAttention attn_output contains NaN or Inf")
            ...
            del attn_output, attn_weights, ffn_output
...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, batch_idx=0):
        attn_output, attn_weights = self.attention(query, key, key)
        if torch.isnan(attn_output).any() or torch.isinf(attn_output).any():
            logger.warning(f"Batch {batch_idx}: CrossAttention attn_output contains NaN or Inf")
            attn_output = torch.nan_to_num(attn_output, nan=0.0, posinf=1.0, neginf=-1.0)
        output = self.norm(query + attn_output)
        del attn_output, attn_weights
        torch.cuda.empty_cache()
        gc.collect()
        return output
...
class MultiModalNet(nn.Module):
    def __init__(self, disease_cols, kg_embeddings, adjacency_matrix):
        ...
        self.img_transformer = TransformerAttention(self.feature_dim, num_heads=2, num_layers=1)
        ...
        self.img_disease_proj = nn.Sequential(
            ...
            nn.Dropout(0.5),
            nn.ReLU(),
            nn.Linear(64, self.num_diseases * 128)
        )
    ...
    def initialize_kg_logits(self):
        ...
        logger.info(f"kg_projection device: {device}")
        logger.info(f"kg_embeddings device: {self.kg_embeddings.device}")
        with torch.no_grad():
    ...
    def forward(self, paired_img, text_feature, meta, use_text=True, batch_idx=0):
        ...
        if use_text and text_feature is not None:
            ...
            if text_feat.dim() != 2:
                raise ValueError(f"Expected text_feature to be 2D [batch_size, 768], got shape {text_feat.shape}")
            text_feat = self.text_proj(text_feat)
            text_feat = self.feat_adapter(text_feat)
            text_feat = text_feat.unsqueeze(0)
            text_feat = text_feat.expand(seq_len, -1, -1)
        ...
        if torch.isnan(global_feat_weighted).any() or torch.isinf(global_feat_weighted).any():
            logger.warning(f"Batch {batch_idx}: global_feat_weighted contains NaN or Inf, attn_sum={attn_sum}")
        ...
        kg_features = torch.matmul(self.A.to_dense() if self.A.is_sparse else self.A, self.kg_embeddings)
        kg_features = self.gcn(kg_features)
        kg_features = F.relu(kg_features)

        img_disease_feat = self.img_disease_proj(global_feat_weighted).view(batch_size, self.num_diseases, 128)
        kg_fused = kg_features.unsqueeze(0) + img_disease_feat
        kg_fused_flat = kg_fused.view(batch_size, -1)

        fused = torch.cat([global_feat_weighted, meta_feat, kg_fused_flat], dim=1)
        logits = self.classifier(fused)

        kg_logits = self.kg_logits.unsqueeze(0).expand(batch_size, -1)

        del global_feat, meta_feat, kg_features, img_disease_feat, kg_fused, kg_fused_flat, fused, attn_map, global_feat_weighted
        torch.cuda.empty_cache()
        gc.collect()

        locals().clear()

        return logits, None, kg_logits, None, None
